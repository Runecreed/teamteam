...
class NS:
    ...
    def trip_list(self, parameters):
        ...
        try:            # see if the data is useful
            station_stops = jsonData['ReisMogelijkheid'][0]['ReisDeel']['ReisStop']

            for station in station_stops:
                list.append(station['Naam'])
        ...
        return {'trips': jsonData["ReisMogelijkheid"], 'station_intermediate': list}        # returns a list of possible trips that go from source to destination, with a lot of info
